...
`proj=agesdk.dev`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=agesdk.dev` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=agesdk.dev` | Start the dev server, skipping install. |
| `make test proj=agesdk.dev` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=agesdk.dev
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy agesdk.dev"` runs `gh repo create` and
...
- **Live URL:** https://agesdk.dev/  *(update once first deploy succeeds)*
...
- [ ] Verify in **Google Search Console** at https://search.google.com/search-console — add as `sc-domain:agesdk.dev` property; verify via DNS TXT record. Until this is done, no SEO traffic data is observable for this site (and the workspace-wide `30 commercial sites with traffic` goal can't credit it).
- [ ] Submit the sitemap (`https://agesdk.dev/sitemap.xml`) inside GSC.
- [ ] Update the **Live URL** above with the actual deploy URL.
- [ ] Run `make run ARGS="cleanup"` from `sites/portfolio/` so `data/portfolio.json` reflects the new project's state (and `project status agesdk.dev` resolves cleanly).
...
`portfolio project check agesdk.dev` (run from `sites/portfolio/`).
